...
from dimos.models.vl.create_vl_model import create_vl_model
...
class PerceiveLoopSkill(Module):
    ...
    _period: float = 2.0  # seconds - how often to run the perceive loop
    ...
    def __init__(
        ...
        self._vl_model: VlModel = create_vl_model(cfg.detection_model)
    ...
    def look_out_for(self, description_of_things: list[str]) -> str:
        ...
        with self._lock:
            ...
            sharpest = sharpness_window(1.0 / self._period, self.color_image.pure_observable())
    ...
    def _on_image(self, image: Image) -> None:
        with self._lock:
            ...
            prompt = _describe_prompt(active_lookout, detections)
            deets = self._vl_model.query(image, prompt)
        ...
        self._agent_spec.add_message(
            HumanMessage(
                f"Found a match for {active_lookout}. Please announce and describe audibly. "
                f"Here are more details. {deets}"
            )
        )
...
def _describe_prompt(
    description_of_things: tuple[str, ...], detections: ImageDetections2D[Detection2DBBox]
) -> str:
    if len(detections) == 1:
        return (
            f"This image has been described as containing a match for the following query: {json.dumps(description_of_things)}. "
            f"The detected object is at this bounding box: {_bounding_boxes_to_str(detections[0])}. "
            f"Please describe the detected match in more detail and describe what part of the query best describes it."
        )

    return (
        f"This image has been described as containing multiple matches for the following query: {json.dumps(description_of_things)}. "
        f"These are the positions of the detected objects: {', '.join([_bounding_boxes_to_str(det) for det in detections])}. "
        f"Please describe the detected matches in more detail and describe what part of the query best describes each one."
    )


def _bounding_boxes_to_str(detection: Detection2DBBox) -> str:
    return json.dumps(dict(zip(["x1", "y1", "x2", "y2"], map(int, detection.bbox), strict=True)))
